# 🧾 **PRD — Sistema de Promociones Multiplataforma (USD Único)**

## 1. Objetivo

Crear un sistema centralizado que permita **definir, activar y finalizar promociones** de productos en múltiples plataformas (Stripe, Xsolla, PayPal, Google Play) desde un único backend, asegurando sincronización automática de precios, coherencia visual en todas las tiendas y rollback controlado.
...
2. Por cada SKU, se sincroniza con las plataformas:
   ...
   - **Google Play** → actualiza el `inappProduct` al precio final.
...
✅ Sincronización consistente entre Stripe, Xsolla, PayPal y Google Play.
...
│   └── product_seed.rs
└── bin/
    ├── worker.rs             # activation / ending / retry / reconcile
    └── seed.rs               # carga inicial
```
